-- ============================================================
-- Migration: Atomic seat reservation RPC
-- ============================================================
...
LANGUAGE plpgsql
SECURITY DEFINER
-- Search path locked for security — prevents schema injection
...
BEGIN
  ...
  -- Loop handles the astronomically rare md5 collision
  LOOP
    v_pnr := upper(substring(md5(random()::text || clock_timestamp()::text), 1, 6));
    EXIT WHEN NOT EXISTS (
      SELECT 1 FROM public.bookings WHERE pnr_code = v_pnr
    );
